---
description: For POs/BAs. Transforms architect specs into backlog—epics (with UXR mapping), wireframes (if UI), and INVEST-compliant user stories.
...
Orchestrates actionable backlog creation: epics with requirement mappings, wireframes (when figma_spec.md exists), and INVEST-compliant user stories with acceptance criteria.

**Prerequisite**: Run `/discovery-agent` first to generate spec.md, design.md, and figma_spec.md (if UI impact).

**Handoff**: Run `/build-agent` to implement user stories with code, and tests.
...
- **If figma_spec.md EXISTS**: UI impact confirmed, execute 3-step workflow (Build Backlog → Generate Wireframes → Refine Stories)
- **If figma_spec.md DOES NOT EXIST**: No UI impact, execute 2-step workflow (Build Backlog → Refine Stories)
...
- Print progress after each step (e.g., "Step 1/3 completed", "Step 3/3 completed" for UI path)
...
- **Progress**: Print "Step 1/N completed: Build Backlog" (N = 3 if figma_spec.md exists, 2 otherwise)
...
2. IF figma_spec.md EXISTS:
   ...
   b. IF GENERATE_WIREFRAMES = true:
      - Print "Wireframe generation ENABLED - Executing 3-step workflow:"
   ...
   c. ELSE (GENERATE_WIREFRAMES = false - default):
      - Print "Wireframe generation DISABLED (default) - Executing 2-step workflow:"
      - Print "  Step 1: Build Backlog (complete)"
...
3. ELSE (no figma_spec.md):
   ...
   - Print "Executing 2-step workflow (Build Backlog → Refine Stories)"
...
- **Progress**: Print "Step 2/3 completed: Generate Wireframes"
...
- **Progress**: Print "Step 3/3 completed: Refine Stories"
...
- **Summarize**: Present key findings including:
  ...
  - Acceptance criteria coverage (Given/When/Then format)
  - Epic coverage percentage
- **Progress**: Print "Step 2/2 completed: Refine Stories"
...
- Report workflow path executed:
  - "3-step UI workflow (with wireframe generation)" if --generate-wireframes used
  - "2-step UI workflow (with wireframe placeholders)" if UI detected but wireframes skipped
  - "2-step Non-UI workflow" if no UI impact
...
- **Next Step**: Recommend running `/build-agent [us_XXX]` to implement user stories with code, tests, and pull requests
...
*Backlog orchestrator for POs/BAs. Consumes architect outputs (spec.md, design.md, figma_spec.md) to produce epics.md, wireframes (if figma_spec.md exists), and user stories. Hand off to `/build-agent` for developer-driven implementation.*
